...
% --------------------------------------------------------------------
function loadnoise_menu_Callback(h, eventdata, handles)
...
[fname, path]=uigetfile({'*.mat', 'ns_thr_polytrode; *.mat'}, 'Select the unit file');

load([path fname], 't_dp_thr', 'thr_step', 'ch_id', 'par');
...
% if ~isfield(handles, 'thr')
    ...
    handles.thr(1).path=path;
% else
%     ind = size(handles.thr,2);
%     handles.thr(ind + 1).t_dp_thr = t_dp_thr; 
%     handles.thr(ind + 1).thr_step = thr_step;
%     handles.thr(ind + 1).type = par.detection;
%     handles.thr(ind + 1).ch_id = ch_id;
%     handles.thr(ind + 1).filename=fname;
%     handles.thr(ind + 1).path=path;
% end
